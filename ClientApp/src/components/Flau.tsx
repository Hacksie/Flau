    ...
    public render() {
        ...
        return (
            <React.Fragment>
                ...
                <Row>
                    {this.props.stages.map((stage: StagesStore.Stage) =>

                        <Col key={stage.id} sm="4" className="p-2 stage">
                            <h1>{stage.desc}</h1>
                            {/* createRef to focus element */}
                            {this.props.items.filter((item) => item.stageId === stage.id).map((item) => (
                                <div key={item.id} onClick={() => { this.props.select(item.id) }} className={`item ${this.props.selectedId === item.id ? "selected" : ""}`}>

                                    <span className="check" onClick={() => this.props.next(item.id)}>
